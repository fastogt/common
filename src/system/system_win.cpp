...
Error Shutdown(shutdown_t type) {
  ...
  if (!is_ok) {
    return make_error_value_perror("OpenProcessToken", errno, SYSTEM_ERRNO, ErrorValue::E_ERROR);
  }
  ...
  if (!is_ok) {
    return make_error_value_perror("LookupPrivilegeValue", errno, SYSTEM_ERRNO, ErrorValue::E_ERROR);
  }

  // get the shutdown privilege for this process
  is_ok = AdjustTokenPrivileges(hToken, FALSE, &tkp, 0, (PTOKEN_PRIVILEGES)NULL, 0);
  if (!is_ok) {
    return make_error_value_perror("AdjustTokenPrivileges", errno, SYSTEM_ERRNO, ErrorValue::E_ERROR);
  }
