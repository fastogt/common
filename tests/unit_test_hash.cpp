...
TEST(hash, md5) {
  ...
  std::string hexed;
  bool is_ok = common::utils::hex::encode(std::string(md5_result, md5_result + MD5_HASH_LENGHT), true, &hexed);
  ASSERT_TRUE(is_ok);
...
TEST(hash, sha1) {
  ...
  std::string hexed;
  bool is_ok = common::utils::hex::encode(sha1_str, true, &hexed);
  ASSERT_TRUE(is_ok);
...
TEST(hash, sha256) {
  ...
  std::string hexed;
  bool is_ok = common::utils::hex::encode(std::string(sha256_result, sha256_result + SHA256_HASH_LENGHT), true, &hexed);
  ASSERT_TRUE(is_ok);
