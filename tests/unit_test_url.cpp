...
TEST(Upath, isValidPathAndQuery) {
  ...
  ASSERT_EQ(file_path, path3.Path());
...
TEST(Uri, Scheme) {
  ...
  ASSERT_EQ(http_path.Hpath(), "hls/69_avformat_test_alex_2/");
  ASSERT_EQ(http_path.Path(), "hls/69_avformat_test_alex_2/play.m3u8");
  ASSERT_EQ(http_path.Filename(), "play.m3u8");
  ASSERT_EQ(http_path.GetUpath(), "hls/69_avformat_test_alex_2/play.m3u8");
  ASSERT_EQ(http_path.Path(), http_path.Hpath() + http_path.Filename());
  ...
  ASSERT_EQ(ftp_path.Hpath(), std::string());
  ASSERT_EQ(ftp_path.Path(), std::string());
  ASSERT_EQ(ftp_path.Filename(), std::string());
  ASSERT_EQ(ftp_path.GetUpath(), std::string());
  ASSERT_EQ(ftp_path.Path(), ftp_path.Hpath() + ftp_path.Filename());
...
TEST(Uri, level) {
  ...
  size_t lv = p.Levels();
  ASSERT_EQ(lv, 2);
  std::string l1 = p.HpathLevel(1);
  ASSERT_EQ(l1, "hls/");
  std::string l2 = p.HpathLevel(2);
